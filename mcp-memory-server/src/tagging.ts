...
export interface EnrichedMetadata {
  ...
  messageType: string;    // question, code-request, explanation, etc.

  // Content analysis
  topics: string[];       // Detected topics
  tags: string[];         // Auto-generated tags
  tools: string[];        // Tools used (for assistant messages)
  ...
  // Content (truncated for metadata)
  content: string;
 ...
 */
function getTemporalBuckets(date: Date): { date: string; week: string; month: string; quarter: string } {
 ...
 */
function detectTopics(contentLower: string): string[] {
 ...
 * Detect message type
 */
function detectMessageType(content: string, role: string): string {
  if (role === "assistant") {
    return "response";
  }
 ...
 */
function detectError(contentLower: string): boolean {
  const errorPatterns = [
    "error",
    "exception",
    "failed",
 ...
 */
function assessComplexity(content: string, hasCode: boolean): "simple" | "medium" | "complex" {
 ...
 */
export function getAvailableMessageTypes(): string[] {
