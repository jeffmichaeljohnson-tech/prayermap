...
import { Activity, AlertTriangle, CheckCircle, Monitor, Cpu, Memory } from 'lucide-react';
import { animationMonitor } from '../lib/animation-monitor';
import type { AnimationMetrics } from '../lib/animation-monitor';
...
export function AnimationPerformanceDashboard({ isVisible, onClose }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    const updateInterval = setInterval(() => {
      const active = animationMonitor.getActiveAnimations();
      setActiveAnimations(active);

      const newMetrics: Record<string, AnimationMetrics> = {};
      active.forEach(animName => {
        const metric = animationMonitor.getMetrics(animName);
        if (metric) {
          newMetrics[animName] = metric;
        }
      });
