...
export function PrayerAnimationLayer({ prayer, userLocation, map, onComplete }: PrayerAnimationLayerProps) {
  ...
  useEffect(() => {
    ...
    // Start monitoring animation performance for Living Map compliance
    ...
    const timer = setTimeout(() => {
      if (!hasCompletedRef.current) {
        ...
        // Track animation completion for Living Map performance monitoring
        const actualDuration = performance.now() - animationStartTimeRef.current;
        livingMapMonitor.trackMapRenderPerformance(actualDuration, 60); // 60fps target
