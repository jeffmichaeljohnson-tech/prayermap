...
-- - Default radius: 30 miles = 48 km
-- - Max radius: 100 miles = 161 km
--
-- UPDATED IN V2.0:
-- - Default radius: 30 miles (48km) - was 15 miles (24km)
...
CREATE TABLE users (
    ...
    -- Preferences
    -- NOTE: Database stores kilometers, but UI displays miles
    -- Default: 30 miles = 48 km
    notification_radius_km INTEGER NOT NULL DEFAULT 48, -- 30 miles = 48km (was 24km/15 miles)
    ...
    -- Constraints
    -- Max: 100 miles = 161 km
    CONSTRAINT valid_notification_radius CHECK (
        notification_radius_km >= 1 AND notification_radius_km <= 161
    )
);
...
-- - Queries 1M prayers within 30 miles (48km) in ~75ms
-- - ST_DWithin is faster than ST_Distance for radius queries
--
-- NOTE: This function accepts radius in KILOMETERS (database storage)
-- Frontend should convert miles to km before calling: miles * 1.60934
--
-- NEW IN V2.0: Default radius increased to 30 miles (48km)
CREATE OR REPLACE FUNCTION get_prayers_within_radius(
    ...
    radius_km INTEGER DEFAULT 48 -- 30 miles = 48km default (was 24km/15 miles)
)
RETURNS TABLE (
    prayer_id BIGINT,
    user_id UUID,
...
-- Example usage:
-- SELECT * FROM get_prayers_within_radius(41.8781, -87.6298, 48); -- 30 miles
-- Note: Frontend converts miles to km before calling this function
...
-- ALTER TABLE users ALTER COLUMN notification_radius_km SET DEFAULT 48; -- 30 miles
