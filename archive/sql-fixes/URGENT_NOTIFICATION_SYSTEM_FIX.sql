-- ============================================================================
-- URGENT NOTIFICATION SYSTEM FIX
-- This SQL can be run directly in Supabase SQL Editor to enable inbox messaging
-- ============================================================================
...
BEGIN
  ...
  -- Get responder name
  SELECT COALESCE(display_name, email, 'Anonymous') INTO responder_name
  FROM profiles 
  ...
  notification_message := responder_name || ' prayed for your prayer: "' || 
                         COALESCE(left(prayer_title, 50), 'Your prayer request') || 
                         CASE WHEN length(prayer_title) > 50 THEN '..."' ELSE '"' END;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'create_prayer_response_notification') THEN
    test_result := '✓ Trigger function created successfully';
  ELSE
    test_result := '✗ Trigger function creation failed';
  END IF;
...
SELECT 'URGENT NOTIFICATION SYSTEM FIX APPLIED SUCCESSFULLY! Inbox messaging should now work.' as status;